...
def test_model(fit_model, model_name, testset, trainset):
	...
	ax.scatter(X_test.iloc[:, 0], X_test.iloc[:, 4], y_test, color='blue')
	#mn, mx = X.iloc[:, 0].min(), X.iloc[:, 0].max()
	#grad = np.linspace(mn, mx, 200)
	dt = pd.DataFrame({'0':np.array(X_test.iloc[:, 0]),'.5':np.array(X_test.iloc[:, 4]), '1':pd.Series(y_pred)})
	dt.sort_values(by='0', inplace=True)
	#print(dt)
	#plt.plot(X_train.iloc[:, 0], pd.Series(y_pred_train), color='red')
	ax.plot(dt.iloc[:, 0], dt.iloc[:, 1], dt.iloc[:, 2], color='red')
	#dt1 = pd.DataFrame({'0':np.array(X_test.iloc[:, 4]), '1':pd.Series(y_pred)})
	#dt1.sort_values(by='0', inplace=True)
	#plt.plot(dt1.iloc[:, 0], dt1.iloc[:, 1], color='green')
	plt.title(f'{model_name} Model Prediction for Post-op HKA from Pre-op HKA')
	plt.xlabel('Pre-op HKA')
	plt.ylabel('Femoral Coronal Rotation')
	ax.set_zlabel('Predicted Planned HKA')
	plt.show()
	plt.close()
